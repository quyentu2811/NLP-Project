...
class DataTokenizingStrategy(DataStrategy):
    ...
    def preprocess_function(self, data: Dataset, *args) -> Dataset:
        ...
        data["input_ids"] = self.tokenizer(inputs, padding="max_length", truncation=True, max_length=2048, return_tensors="pt").input_ids
        data["attention_mask"] = self.tokenizer(inputs, padding="max_length", truncation=True, max_length=2048, return_tensors="pt").attention_mask
        data["labels"] = self.tokenizer(data["summary"], padding="max_length", truncation=True, max_length=2048, return_tensors="pt").input_ids
